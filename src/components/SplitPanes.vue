...
// On splitter dbl click or dbl tap, toggle pane visibility based on relative sizes.
const onSplitterDblClick = (event, splitterIndex) => {
  ...
  if (props.maximizePanes) {
    const leftPane = panes.value[splitterIndex - 1]
    const rightPane = panes.value[splitterIndex]
...
.splitpanes {
  ...
  &__splitter {
    ...
    &:focus {outline: none;}
    &:hover, &:active {background: rgb(var(--v-theme-primary));}
  }
  &--vertical > .splitpanes__splitter {width: 6px;min-width: 5px;cursor: col-resize;}
  &--horizontal > .splitpanes__splitter {height: 6px;min-height: 5px;cursor: row-resize;}
}
</style>
