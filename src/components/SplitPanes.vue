  ...
  if (paneBeforeMaxReached || paneAfterMaxReached) {
    if (paneBeforeMaxReached) {
      ...
      paneAfter.size = Math.min(
        Math.max(100 - paneBefore.max - sums.prevPanesSize - sums.nextPanesSize, paneAfter.min),
        paneAfter.max
      );
    } else {
...
const doPushOtherPanes = (sums, dragPercentage) => {
  ...
  if (dragPercentage > 100 - sums.nextPanesSize - panes.value[panesToResize[1]].min) {
    ...
    sums.nextPanesSize =
      panesToResize[1] !== undefined ? sumNextPanesSize(panesToResize[1] - 1) : 0;
...
.splitpanes {
  ...
  *:has(&--dragging) {
  ...
  &__pane {
    ...
    .splitpanes--ready.splitpanes--dragging & {
      transition: none;
    }
  }
